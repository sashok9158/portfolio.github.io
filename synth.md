---
layout: default
---
{% include nav.html %}

# Retro Synth - In Progress!

**Course:** Hands-On PCB Engineering DeCal @ UC Berkeley

As part of a student run project course, I am working with 2 partners to build a functional 12 note analog PCB synth. We are currently in the design phase for this project and I am working on cleaning up our schematic and adding footprints for the board on KiCAD.

**Design**

The first part of the design is the block diagram, which was then used to find parts and put into the KiCAD Schematic. After the design review, I will be updating the schematic and working with my team to order parts to put together the board.

|Block Diagram | Schematic|
|:---------------------------------------|:---------------------------------------|
|![block diagaram](/images/synth-block) | ![synth schematic](/images/synth-schm)|
